...
static void *rendering(void *arg)
{
	...
	//printf("rendering after SET_VSYNC_INT\n");
	...
	while(1) {
		...
		if(disp_flag == 0) {
			pp_param->dst_buf_addr_phy = fb_info.map_dma_f1;	// physical address of FB0
		} else {
			pp_param->dst_buf_addr_phy = fb_info.map_dma_f2;	// physical address of FB1
		}
